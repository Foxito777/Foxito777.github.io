...
public interface ProductoRepository extends JpaRepository<Producto, Long> {
    ...
    // Método para encontrar productos similares por categoría (sin incluir el mismo producto)
    @Query("SELECT p FROM Producto p WHERE p.categoria = :categoria AND p.id != :id AND p.disponible = true")
    List<Producto> findByCategoriaAndIdNot(@Param("categoria") String categoria, @Param("id") Long id);

    // Método para encontrar productos aleatorios (para carrusel general)
    @Query(value = "SELECT * FROM productos WHERE disponible = true ORDER BY RAND()", countQuery = "SELECT count(*) FROM productos WHERE disponible = true", nativeQuery = true)
    Page<Producto> findRandomProducts(Pageable pageable);
}
